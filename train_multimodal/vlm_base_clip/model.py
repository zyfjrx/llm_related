...
class VisionProj(nn.Module):
    def __init__(self, ve_dim=768 * 4, lm_dim=896):
        super().__init__()
        self.ve_dim = ve_dim
        self.lm_dim = lm_dim
...
class VLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.vision_model = CLIPModel.from_pretrained(self.config.vision_model_path)
        self.processor = CLIPProcessor.from_pretrained(self.config.vision_model_path)
        ...
        self.visionProj = VisionProj(lm_dim=self.llm_model.config.hidden_size,ve_dim=self.vision_model.config.vision_config.hidden_size * 4)
