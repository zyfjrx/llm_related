...
def evaluate(dataloader, model, tokenizer,  device):
    ...
    special_tokens = [tokenizer.pad_token_id, tokenizer.eos_token_id, tokenizer.sos_token_id]
    for inputs, targets in tqdm(dataloader, desc='evaluate'):
        ...
        batch_result = predict_batch(inputs, model,tokenizer, device)
...
def run_evaluate():
    ...
    bleu = evaluate(dataloader,model,tokenizer, device)
