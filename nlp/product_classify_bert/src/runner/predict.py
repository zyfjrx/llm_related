...
def predict(user_input, model, tokenizer, device,class_label):
    ...
    return class_label.int2str(batch_result[0])
...
def run_predict():
    ...
    while True:
        ...
        if user_input.strip() == "":
            print("请输入商品标题：(输入q或者quit退出)")
            continue
        result = predict(user_input,model,tokenizer,device,class_label)
        print(result)
