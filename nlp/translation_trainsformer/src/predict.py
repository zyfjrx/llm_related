...
def predict_batch(input_ids, model, en_tokenizer, zh_tokenizer, device):
    """
    ...
    with torch.no_grad():
        # 编码
        src_pad_mask = (input_ids == zh_tokenizer.pad_token_id)
        # memory.shape[batch_size,src_len,d_model]
        memory = model.encode(input_ids,src_pad_mask)
        # 解码
        batch_size = input_ids.shape[0]
        # decoder_input.shape[batch_size,1]
        decoder_input = torch.full(size=(batch_size, 1), fill_value=zh_tokenizer.sos_token_id, dtype=torch.long,
                                   device=device)
...
def predict(user_input, model, en_tokenizer, zh_tokenizer, device):
    # 处理数据
    input_ids = zh_tokenizer.encode(user_input, config.SEQ_LEN, add_special_tokens=False)
    input_ids = torch.tensor([input_ids], dtype=torch.long).to(device)
    output_ids = predict_batch(input_ids, model, en_tokenizer, zh_tokenizer, device)
